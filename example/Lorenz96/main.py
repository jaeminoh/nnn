...
def main(
    ...
    path = "data/Lorenz96"
    fname = f"{path}/{filter_type}_Forcing{int(forcing)}Noise{noise_level}Obs{sensor_every}Nx{Nx}"
    print(f"""
          ==============================
          Configurations:
    ...
    model = Lorenz96(Nx=Nx, inner_steps=assimilation_window, forcing=forcing)
    observe = UniformSubsample(sensor_every=sensor_every)
    ...
    uu = test_on(
        ...
        unroll_length=test_unroll_length,
    )
    ...
    err_baseline = (
        jax.vmap(jnp.linalg.norm)(uu.baseline - uu.reference) / norm_reference
    )
    err_forecast = (
        jax.vmap(jnp.linalg.norm)(uu.forecast - uu.reference) / norm_reference
    )
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig(f"{fname}_err.pdf", dpi=300)
